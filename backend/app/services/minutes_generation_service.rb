class MinutesGenerationService
  ...
  def open_questions
    matches = matching_lines(/\?|question|open question|確認|未決|論点|質問/i)
    matches.map { |text| normalize(text) }
  end

  def action_items
    matches = matching_lines(/todo|action|next|follow|対応|担当|宿題|次回|実施/i)
    matches.map do |text|
      {
        text: normalize(text),
        status: "open"
