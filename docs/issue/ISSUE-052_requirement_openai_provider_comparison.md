...
- `docs/review/20260708_requirement_openai_quota_operator_checklist_review.md`
- `docs/review/20260709_requirement_openai_live_case_001_retry_review.md`
...
2026-07-08追記: safe failure report出力を使って1ケース評価を進めたが、`CASE-RQ-001` は `insufficient_quota` / `too_many_requests` で基準未判定のまま停止した。証跡は `docs/evaluation/20260708_requirement_generation_openai_live_case_001_failure.md` に保存され、API key、Authorization header、raw provider response、request payload全文、model output、PII / credential / tokenは保存していない。Platform側のusage、billing、rate limit、model access確認後に同じcaseから再開する。

2026-07-08追記: partial resume運用を安全にするため、provider失敗時に `--resume-output` でsafe JSON manifestを書き出せるようにした。manifestにはfixture issue/version、provider、selected/completed/next case、safe error、推奨CLI引数だけを保存し、API key、raw provider response、request payload全文、model output、raw request idは保存しない。

2026-07-08追記: `insufficient_quota` / `too_many_requests` から再開するため、`docs/release/20260708_requirement_openai_quota_operator_checklist.md` を追加した。OpenAI Platform側のbilling、usage、limits、model access、API key所属project、`CASE-RQ-001` の低負荷再実行、safe failure別の対応表、クローズ判定を日本語で一本化した。

2026-07-09追記: `.env` 読込条件で `npm run requirements:openai:readiness` を再実行し、`OPENAI_API_KEY` と `OPENAI_REQUIREMENT_MODEL` は設定済み、fixtureと出力先も問題なし、`safe_failures` は空であることを確認した。その後、`CASE-RQ-001` を `--delay-seconds 10`、`--failure-output`、`--resume-output` 付きで低負荷再実行したが、OpenAI APIは再度 `insufficient_quota` / `too_many_requests` を返した。証跡は `docs/evaluation/20260709_requirement_generation_openai_live_case_001_failure.md` と `docs/evaluation/20260709_requirement_generation_openai_live_case_001_resume.json` に保存した。API key、Authorization header、raw provider response、request payload全文、model output、PII / credential / tokenは保存していない。OpenAI出力品質は引き続き基準未判定のため、Issue #69はOPEN継続とする。
...
1. OpenAI Platform側でbilling、usage上限、project budget、rate limit、model access、API key所属projectを再確認する。
2. quota制約が解消した後、`--case-id CASE-RQ-001`、`--delay-seconds 10`、`--failure-output`、`--resume-output` を指定してOpenAI live manual smokeを再実行する。
3. 成功した場合は評価Markdownとreview docを保存し、deterministic providerとの差分評価へ進む。
4. 失敗した場合はsafe failure reportとsafe resume JSONを保存し、Issue #69へ同期する。
