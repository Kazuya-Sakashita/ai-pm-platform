...
- `Link Issue` のinline error、非GitHub URL、http URL、別repository URLのE2Eは実装済み。URL/番号不一致の専用E2Eは未追加。
...
- controlled retryのcooldown、retry count、cooldown後の非同期retry job、承認者入力、理由テンプレートは実装済み。ただし、承認履歴一覧UIと認証ユーザーへの紐付けは未実装。
...
- `Link Issue` のURL/番号不一致専用E2Eを追加する
- controlled retryの承認履歴UIと認証ユーザー紐付け方針を追加する
