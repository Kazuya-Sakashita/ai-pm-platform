...
- callback失敗時にstateを消費済みにする現在仕様はADR-0011で方針決定済み。callback failure AuditLogとワークスペース内のFrontend再接続導線は実装済み。ただしGitHubから戻る専用callback result pageは未実装。
...
- staging worker smokeとqueue監視設計を後続Issueとして確認する
...
- GitHub callback result pageを追加し、GitHubから戻った後の成功/失敗表示を明確にする
