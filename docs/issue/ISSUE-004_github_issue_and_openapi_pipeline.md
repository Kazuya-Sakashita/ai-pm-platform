...
- `docs/review/20260708_smoke_readiness_next_actions_review.md`
- `docs/review/20260708_issue004_live_gate_recheck_review.md`
- `docs/review/20260708_issue004_live_gate_operator_checklist_review.md`
- `docs/review/20260709_issue004_live_gate_recheck_review.md`
...
2026-07-08追記: GitHub webhook live smokeとSolid Queue worker smokeのreadiness scriptへ `next_actions` を追加した。`safe_failures` が空でない場合でも、operatorが次に直すべき設定をsafe JSON内で確認できる。再実行結果ではGitHub webhook側が `github_webhook_secret_missing`、`github_webhook_url_placeholder`、`github_webhook_recent_delivery_failed` のままで、`GITHUB_WEBHOOK_SECRET` 設定、Webhook URL変更、delivery再送が必要であることを確認した。Solid Queue側はlocal developmentで `solid_queue_tables_unavailable` のため、staging/production-equivalent環境でqueue schema適用済み `QUEUE_DATABASE_URL` を設定して再実行する必要がある。いずれもsecret値、raw payload、raw delivery id、DB URLは保存していない。

2026-07-08追記: live gateを再確認した。GitHub webhook smokeはApp ID/private keyは設定済みだが、runtime `GITHUB_WEBHOOK_SECRET` 未設定、GitHub App webhook URLが `https://example.com/github/webhook` のplaceholder、直近deliveryが502のため未合格だった。Solid Queue worker readinessはlocal developmentではActiveJob adapterがasync、Solid Queue table未準備のため、staging/production-equivalent環境の証跡としては未合格である。Issue #4は外部設定と実環境証跡が揃うまでOPEN継続とする。

2026-07-08追記: 外部設定待ちで運用担当者が迷わないよう、`docs/release/20260708_issue004_live_gate_operator_checklist.md` を追加した。GitHub App側のWebhook URL / secret、runtime `GITHUB_WEBHOOK_SECRET`、delivery redeliver、`GithubWebhookDelivery` / AuditLog safe確認、staging / production-equivalent worker readiness、safe failure別の対応表、クローズ判定を日本語で一本化した。

2026-07-09追記: Issue #4のlive gateを再確認した。GitHub webhook live smokeはApp ID/private keyは設定済みだが、runtime `GITHUB_WEBHOOK_SECRET` は未設定、GitHub App webhook URLは `https://example.com/github/webhook` のplaceholder、直近deliveryは502のままだった。safe failuresは `github_webhook_secret_missing`、`github_webhook_url_placeholder`、`github_webhook_recent_delivery_failed`。Solid Queue worker readinessはlocal developmentで実行し、ActiveJob adapterはasync、Solid Queue table未準備のため、`solid_queue_tables_unavailable`、`active_job_adapter_not_solid_queue` で未合格だった。証跡は `docs/evaluation/20260709_issue004_github_webhook_live_smoke_recheck.json`、`docs/evaluation/20260709_issue004_solid_queue_worker_smoke_readiness_local.json`、レビューは `docs/review/20260709_issue004_live_gate_recheck_review.md` に保存した。secret値、raw payload、signature、raw delivery id、DB URLは保存していない。Issue #4は外部設定と実環境証跡が揃うまでOPEN継続とする。
...
- `safe_failures` が残る場合はscript出力の `next_actions` に従って設定を修正する
