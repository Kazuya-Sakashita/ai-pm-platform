...
- GitHub searchのindexing delay、rate limit、retry/backoff方針はADR-0008で定義済みで、response header parsing、retry metadata、cooldown UI、ActiveJobによる非同期retry jobは実装済み。production向け永続Job基盤はADR-0010でSolid Queue採用方針を決め、ISSUE-023で実装済み。production mode local worker smokeも別queue DBで確認済み。Solid Queue staging/production worker smoke runbookとQueue health監視MVPも追加済み。ただし、実staging/productionでのworker smoke証跡、通知/SLO、failed job運用UIは未実装。
...
- controlled retryの承認履歴UIは追加済み。次は認証ユーザー紐付け方針を追加する
- 実credential smoke結果を `docs/review/YYYYMMDD_github_app_live_smoke_review.md` として保存する
- GitHub照合履歴のpagination、filter、監査ログ詳細画面を検討する
- staging/production worker smoke runbookに沿って実環境証跡を取得する
- queue監視の通知/SLO、failed job運用UI、operator権限を後続Issueとして設計する
...
- GitHub callback result pageを実GitHub App setup URLからlive smokeし、query parameterと戻り挙動を確認する
