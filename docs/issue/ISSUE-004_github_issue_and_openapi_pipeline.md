...
- controlled retryのcooldownとretry countは実装済みだが、承認者表示と理由テンプレートは未実装。
- GitHub searchのindexing delay、rate limit、retry/backoff方針はADR-0008で定義済みで、response header parsing、retry metadata、cooldown UIは実装済みだが、ActiveJob等による非同期retry jobは未実装。
...
- ADR-0008に基づきGitHub Search非同期retry jobを実装する
