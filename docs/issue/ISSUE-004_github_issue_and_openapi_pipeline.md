...
差別化に直結するP0。2026-06-30時点で、承認済みRequirementからIssue DraftとOpenAPI Draftを生成、編集、保存できるMVP実装を追加した。2026-07-01時点で、OpenAPI Draftのvalidation API、Frontend validation導線、validation結果とReview blockerの連動、GitHub Issue publish API/gate、GitHub App provider、`integration_accounts`、GitHub connection APIを追加した。
...
- callback時にGitHub APIへinstallation実在確認を行う処理は未実装。
...
レビュー結果: MVPとしては前進。fake provider、GitHub App provider spec、connection API specではGitHub Issue番号とローカル台帳の紐付け経路まで確認済み。ただし実GitHub App credentialによるconnect/publish smokeとinstallation検証が未完了のため、ISSUE-004はクローズ不可。
...
- callback時にGitHub APIでinstallation/repository/permissionsを検証する
