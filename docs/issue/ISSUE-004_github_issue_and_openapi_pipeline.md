...
- `github_issue_publish_attempts` とreconciler serviceはADR定義のみで未実装。
...
レビュー結果: MVPとしては前進。fake provider、GitHub App provider spec、connection API spec、callback verification spec、state nonce specではGitHub Issue番号とローカル台帳の紐付け経路まで確認済み。ADR-0006でpublish reconciliation方針も定義済み。ただし実GitHub App credentialによるconnect/publish smokeとpublish reconciliation実装が未完了のため、ISSUE-004はクローズ不可。
...
- `github_issue_publish_attempts` migration/model/specを追加する
- `GithubIssuePublishService` にattempt記録を接続する
- ambiguous failure用のreconciler serviceを実装する
- 0件/複数件時にReview blockerを作る
