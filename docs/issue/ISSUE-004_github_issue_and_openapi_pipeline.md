...
- callback失敗時にstateを消費済みにする現在仕様はADR-0011で方針決定済み。ただしcallback failure AuditLogとFrontend再接続UXは未実装。
...
- callback failureをAuditLogへ記録し、Frontendに再接続導線を追加する
