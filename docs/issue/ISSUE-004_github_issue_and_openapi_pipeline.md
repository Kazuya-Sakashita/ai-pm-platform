...
- marker検索reconcilerとAPI実行導線は実装済みだが、管理画面からの実行導線は未実装。
- Review blockerから正しいGitHub Issueを手動選択するUI/APIは未実装。
...
レビュー結果: MVPとしては前進。fake provider、GitHub App provider spec、connection API spec、callback verification spec、state nonce spec、publish attempt spec、marker search/reconciler spec、reconciliation API request specではGitHub Issue番号とローカル台帳の紐付け経路、外部side effect監査、reconciliation required停止、exact match復旧、0件/複数件時のReview blocker、APIからのreconciler実行まで確認済み。ADR-0006でpublish reconciliation方針も実装へ進んだ。ただし実GitHub App credentialによるconnect/publish/reconcile smoke、手動紐付け/controlled retry導線が未完了のため、ISSUE-004はクローズ不可。
...
- `issue_draft.github_published` AuditLogへattempt_idを追加する
