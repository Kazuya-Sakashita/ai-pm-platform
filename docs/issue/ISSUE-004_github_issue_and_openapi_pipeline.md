...
- GitHub searchのindexing delay、rate limit、retry/backoff方針はADR-0008で定義済みで、response header parsing、retry metadata、cooldown UI、ActiveJobによる非同期retry jobは実装済み。ただし、production向け永続Job基盤の選定ADRが必要。
...
- production向け永続Job基盤としてSolid Queue等の採否をADR化する
