...
- GitHub searchのindexing delay、rate limit、retry/backoff方針はADR-0008で定義済みで、response header parsing、retry metadata、cooldown UI、ActiveJobによる非同期retry jobは実装済み。production向け永続Job基盤はADR-0010でSolid Queue採用方針を決め、ISSUE-023で実装済み。production mode local worker smokeも別queue DBで確認済み。Solid Queue staging/production worker smoke runbookも追加済み。ただし、実staging/productionでのworker smoke証跡、queue監視、failed job運用UIは未実装。
