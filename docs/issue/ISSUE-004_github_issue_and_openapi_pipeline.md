...
- ADR-0007でmarker検索候補のscore解釈、並び順、最大表示件数は定義済みだが、選択済み状態、aria属性、long title視覚回帰、10件超過表示は未実装。
