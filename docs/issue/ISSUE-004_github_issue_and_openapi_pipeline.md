...
- callback state replay防止のnonce保存は未実装。
...
レビュー結果: MVPとしては前進。fake provider、GitHub App provider spec、connection API spec、callback verification specではGitHub Issue番号とローカル台帳の紐付け経路まで確認済み。ただし実GitHub App credentialによるconnect/publish smoke、state replay防止、reconciliationが未完了のため、ISSUE-004はクローズ不可。
...
- callback state nonce保存を追加し、replayを拒否する
