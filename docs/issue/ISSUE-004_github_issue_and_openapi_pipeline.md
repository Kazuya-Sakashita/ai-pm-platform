...
差別化に直結するP0。2026-06-30時点で、承認済みRequirementからIssue DraftとOpenAPI Draftを生成、編集、保存できるMVP実装を追加した。2026-07-01時点で、OpenAPI Draftのvalidation API、Frontend validation導線、validation結果とReview blockerの連動、GitHub Issue publish API/gate、GitHub App provider、`integration_accounts` を追加した。
...
- GitHub App providerは実装済みだが、GitHub App installation callback/APIとlive credentialでの公開検証は未実施。
...
レビュー結果: MVPとしては前進。fake providerとGitHub App provider specではGitHub Issue番号とローカル台帳の紐付けまで確認済み。ただしGitHub App installation flowとlive publish検証が未完了のため、ISSUE-004はクローズ不可。
...
- GitHub App installation callback/APIを実装し、`integration_accounts` を自動作成する
- 実GitHub App credentialを使ったstaging/live publish smokeを実施する
