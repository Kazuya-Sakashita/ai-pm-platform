...
- marker検索候補にはtitle/state/updated_at/scoreがなく、選択判断材料がまだ薄い。
...
- marker検索候補へtitle/state/updated_at/scoreなどの判断材料を追加する
