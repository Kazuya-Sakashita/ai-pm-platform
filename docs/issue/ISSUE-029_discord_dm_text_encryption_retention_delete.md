...
- project membership導入後のPolicy Object設計と初期実装がある
...
- `docs/review/20260706_discord_dm_data_protection_parent_closure_review.md`
...
2026-07-06に親Issueクローズ判定レビューを実施した。子Issue #30、#31、#32、#33、#34、#35、#38 がすべてCLOSEDであり、DM本文暗号化、retention、匿名化API、safe AuditLog、Policy Object、summary draft派生JSON保護、PII検出、失敗系E2Eの実装条件を満たしたため、ISSUE-029をクローズ可能と判定した。実staging/production worker smoke、KMS実接続、backup retention provider設定、外部AIレビュー比較はrelease gateまたはISSUE-004側の残リスクとして継続する。
...
- 実認証/JWT接続とmembership管理APIは未実装。
- Project membership認可はDM系APIへ限定実装であり、他APIへの横展開は未完了。
...
4. ISSUE-033でretention jobをstaging worker smoke runbookへ追加する（2026-07-05完了、GitHub #33クローズ済み。実staging/prod証跡はrelease gateへ移管）。
5. ISSUE-034でFrontendの匿名化失敗、キャンセル、権限エラー、モバイル表示E2Eを追加する（2026-07-05完了、GitHub #34クローズ済み）。
6. ISSUE-029親Issueをクローズする（2026-07-06完了）。
