...
- 本Issue内で `docs/review/YYYYMMDD_expert_subagent_governance_review.md` を作成予定
...
1. GitHub Issue #41は同期済み。
2. `docs/ai/expert_subagents.md` を作成する。
3. `docs/evaluation/expert_review_schema.md` を作成する。
4. 専門家サブエージェント運用ADRを作成する。
5. AGENTS.mdへ専門家サブエージェント運用ルールを追記する。
6. ISSUE-039でパイロットレビューを実施する。
