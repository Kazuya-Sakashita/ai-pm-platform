# Workflow endpoint auth coverage design review
...
古いworkflow endpointsの認証/認可差分をなくし、ProjectMembershipを一貫したproject authorization境界にする。
...
- auth coverage matrixを `docs/security/` に保存する。
...
- P1: project role policyを共通化し、workflow endpointsへ適用する。
...
1. authorization helperを追加する。
2. workflow controllersへproject role gateを適用する。
3. request specsを更新する。
4. OpenAPIを更新する。
5. Security/QA implementation reviewを保存する。
