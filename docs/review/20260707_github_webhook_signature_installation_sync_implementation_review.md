...
対象RSpec、GitHub連携周辺回帰、Zeitwerk、OpenAPI検証が成功した。Security Engineer観点のP0指摘である「署名検証前の副作用禁止」「delivery冪等性」「safe metadata」「権限失効同期」は実装で採用した。
...
- `bundle exec rspec spec/services/github_integration/webhook_signature_verifier_spec.rb spec/requests/api/v1/webhooks_spec.rb`: 13 examples, 0 failures
...
- `bundle exec rspec`: 388 examples, 0 failures
