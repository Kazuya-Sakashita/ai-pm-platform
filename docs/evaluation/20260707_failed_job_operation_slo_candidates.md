...
| retry後再失敗率 | 10%未満 | AuditLog `operations.failed_job_retried` と `job_queue_mappings`、Solid Queue failed execution | warning |
...
- `retry_refailure_rate` はISSUE-066で実測値へ更新し、10%以上でwarningとする。
