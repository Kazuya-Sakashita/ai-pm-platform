...
- [Done] `conversation_imports.raw_text` / `redacted_text` の暗号化方式を実装する。
- [Done] retention policyと削除/匿名化APIをOpenAPIへ追加する。
- [Done] retention jobをSolid Queueで実装する。
- [Done 2026-07-05] key rotation、KMS、backup削除方針をADR-0013で定義する。
- [Todo] retention jobをstaging smoke対象に含める。
